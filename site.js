// Toddler Kitties — site interactions
...
document.addEventListener('click', function (e) {
  ...
  function evLaser() {
    const success = coin();
    const dot = fx('fx-laser', rect.left - 90, rect.bottom - 20);
    ...
    setTimeout(function () {
      if (success) { dot.style.opacity = '0'; cat.classList.remove('walking'); happyHop(function () { dashOff(0); }); }
      else { cat.classList.remove('walking'); cat.classList.add('skid'); dashOff(300); }
    }, 470);
  }
  ...
  function evFly() {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        if (success) flyEl.style.opacity = '0';
        else flyEl.style.transform = 'translate(140px, -180px)';
        setMove(0, 0, '0.2', 'ease-in');
        setTimeout(function () {
  ...
  function evBird() {
    const success = coin();
    const bird = fx('fx-bird', rect.left - 140, rect.top - 40);
    requestAnimationFrame(function () {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        if (success) bird.style.opacity = '0';
        else bird.style.transform = 'translate(' + window.innerWidth + 'px, -220px)';
  ...
  function evScratch() { cat.classList.add('scratching');
    setTimeout(function () { cat.classList.remove('scratching'); dashOff(60); }, 820); }
  function evLick() { cat.classList.add('licking');
  ...
  function evBat() {
    ...
    setTimeout(function () { cat.classList.remove('batting'); dashOff(0); }, 520);
  }
...
// hero cat hops when tapped
document.addEventListener('click', function (e) {
  const hero = e.target.closest('#hero-cat');
  if (!hero) return;
  const svg = hero.querySelector('svg');
  if (svg) {
    svg.style.transition = 'transform 0.16s ease-out';
    svg.style.transform = 'translateY(-14px)';
    setTimeout(function () { svg.style.transform = 'translateY(0)'; }, 170);
  }
});
